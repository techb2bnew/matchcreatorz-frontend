...
export default function BuyerAccountPage() {
  ...
  const initials = form.name.split(' ').map((w: string) => w[0]).slice(0, 2).join('').toUpperCase() || 'B';
  ...
  const savePassword = async () => {
    ...
    try {
      await profileApi.changePassword('buyer', { user_id: userId, current_password: passwords.current, new_password: passwords.newPass });
            ...
            {loading ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="sm:col-span-2">
                  ...
                  <textarea className={inputCls + ' resize-none py-3'} rows={3} value={form.bio} onChange={e => setForm(f => ({ ...f, bio: e.target.value }))} placeholder="Tell sellers about yourself..." />
                </div>
