...
export default function AdminProfilePage() {
  ...
  const initials = form.name.split(' ').map((w: string) => w[0]).slice(0, 2).join('').toUpperCase() || 'A';

  const saveProfile = async () => {
    setProfSaving(true); setProfMsg(null);
    try {
      await profileApi.update('admin', { user_id: userId, name: form.name, phone: form.phone, bio: form.bio, location: form.location });
  ...
  const savePassword = async () => {
    ...
    try {
      await profileApi.changePassword('admin', { user_id: userId, current_password: passwords.current, new_password: passwords.newPass });
            ...
            {loading ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input className={inputCls + ' bg-gray-50 cursor-not-allowed'} type="email" value={form.email} readOnly />
                </div>
                <div>
                ...
                <div className="sm:col-span-2">
                  ...
                  <textarea className={inputCls + ' resize-none py-3'} rows={3} value={form.bio} onChange={e => setForm(f => ({ ...f, bio: e.target.value }))} placeholder="About you..." />
                </div>
